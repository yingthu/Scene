...
public class OneFourViewPanel extends MultiViewPanel {
	...
	public void reshapeViews()
	{
		...
		if (showFour)
		{
			...
			topController.reshape(0, halfHeight, halfWidth, height - halfHeight);
			frontController.reshape(halfWidth, halfHeight, width - halfWidth, height - halfHeight);
			rightController.reshape(0, 0, halfWidth, halfHeight);
			perspectiveController.reshape(halfWidth, 0, width - halfWidth, halfHeight);
			
			horizontalBar.reshape(0, halfHeight - 1, width, 2);
			verticalBar.reshape(halfWidth - 1, 0, 2, height);
		}
		else
		{
			perspectiveController.reshape(0, 0, width, height);
		}
	}
